...
const MusicSection = () => {
  ...
  if (loading) {
    return (
      <section id="music" className="py-16 px-4 sm:px-6 bg-[var(--almost-black)] min-h-[40vh] flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin w-12 h-12 border-2 border-gray-800 border-t-transparent rounded-full mx-auto mb-4"></div>
          <p className="text-gray-300 text-xl">Loading vinyl collection...</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section id="music" className="py-12 px-4 sm:px-6 bg-[var(--almost-black)] relative overflow-hidden">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          <h2 className="custom-font text-2xl sm:text-3xl font-bold mb-3">
            <span className="text-[var(--off-white)]">Music</span>
            <span className="text-gray-300"> Production</span>
          </h2>
          <p className="coolvetica-font text-base sm:text-lg text-gray-300 max-w-2xl mx-auto mb-5">
        ...
        {apiSource === 'combined' && credits.length > 0 && (
          <div className="bg-amber-900/20 border border-amber-700/50 rounded-lg p-3 mb-5 text-center">
        ...
        {credits.length > 0 ? (
          <>
            ...
            {viewMode === 'vinyl' && (
              <div className="relative">
                {/* Carousel Container */}
                <div className="relative h-72 flex items-start justify-center overflow-hidden mb-5 pt-4">
                  <div
                    ref={trackRef}
                    className="flex gap-5 absolute left-1/2 cursor-grab active:cursor-grabbing select-none"
                    style={{
                              ...
                              <div className="absolute bottom-4 left-4 right-4">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  {track.source === 'soundcloud' && (
                                    <span className="text-xs bg-amber-500 text-white px-2 py-1 rounded">SC</span>
                                  )}
                                  {(track.source === 'spotify' || track.spotifyUrl) && (
                                    <span className="text-xs bg-amber-500 text-white px-2 py-1 rounded">SP</span>
                                  )}
                                </div>
                              </div>
                            </div>
                ...
                {/* Instructions */}
                <div className="text-center">
                  <p className="text-gray-400 text-sm">Drag to browse • Click to explore</p>
                </div>
              </div>
            )}
            ...
            {viewMode === 'list' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
                {credits.map((credit, i) => (
                  <div
                    key={i}
                    className="bg-gray-900/40 border border-gray-800/60 rounded-xl p-4 hover:border-amber-400/60 hover:shadow-lg transition-all duration-300 group cursor-pointer"
                  ...
                  >
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-grow">
                        <h4 className="text-xl font-semibold text-[var(--off-white)] mb-2">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        className="bg-amber-600 hover:bg-amber-700 !text-white visited:!text-white hover:!text-white focus:!text-white no-underline px-3 py-2 rounded-lg text-sm flex items-center justify-center transition-colors"
                          >
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-800/60">
                    ...
                    <span className="text-xs text-gray-100 bg-gray-800/50 px-3 py-1 rounded-full">
                          {credit.customGenre || credit.genre}
                        </span>
                      )}
                  ...
                  <span className="text-xs text-gray-400">
                    ...
                    {credit.description && (
                  <p className="text-gray-300 text-sm mt-3 pt-3 border-t border-gray-800/60">
                        {credit.description}
                      </p>
                    )}
                  </div>
          ...
          <div className="text-center py-12">
            <div className="w-24 h-24 bg-gray-900/30 rounded-2xl flex items-center justify-center mx-auto mb-6">
              <Music className="w-12 h-12 text-gray-400" />
            </div>
            <h3 className="text-xl font-semibold text-[var(--off-white)] mb-3">No Music Credits Found</h3>
      ...
      {selectedTrack && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-fadeIn">
          <div className="bg-gray-950/95 rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden relative animate-slideUp">
            {/* Close Button */}
            <button
              onClick={closeTrackDetails}
              className="absolute top-6 right-6 z-10 w-12 h-12 bg-gray-800/70 hover:bg-gray-700 rounded-full flex items-center justify-center transition-colors"
            >
              <X className="w-6 h-6 text-[var(--off-white)]" />
            </button>
            ...
            <div className="flex flex-col lg:flex-row">
                ...
                <div className="relative animate-vinylSlideIn">
                  ...
                  <div className="w-80 h-80 bg-black rounded-full shadow-2xl relative overflow-hidden animate-vinylSpin">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-16 h-16 bg-gray-900/30 rounded-full flex items-center justify-center">
                        <div className="w-4 h-4 bg-amber-400/80 rounded-full"></div>
                      </div>
                    </div>
                    {/* Vinyl Ridges */}
                    {[1, 2, 3, 4, 5].map(i => (
              ...
              <div className="lg:w-1/2 p-8 overflow-y-auto animate-slideInRight">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="w-10 h-10 bg-gray-800/50 hover:bg-gray-800 rounded-full flex items-center justify-center transition-colors"
                    >
                      <ChevronLeft className="w-5 h-5 text-[var(--off-white)]" />
                    </button>
                    ...
                    <button
                      ...
                      className="w-10 h-10 bg-gray-800/50 hover:bg-gray-800 rounded-full flex items-center justify-center transition-colors"
                    >
                      <ChevronRight className="w-5 h-5 text-[var(--off-white)]" />
                    </button>
                ...
                <div className="space-y-6">
                  <div>
                  <h2 className="text-3xl font-bold text-[var(--off-white)] mb-2">{selectedTrack.trackTitle || selectedTrack.title}</h2>
                  <p className="text-gray-200 text-lg font-medium">{selectedTrack.artist}</p>
                    <div className="flex items-center gap-2 mt-2">
                      {selectedTrack.source === 'soundcloud' && (
                      <span className="text-xs bg-amber-500 text-white px-2 py-1 rounded">SoundCloud</span>
                      )}
                      {(selectedTrack.source === 'spotify' || selectedTrack.spotifyUrl) && (
                      <span className="text-xs bg-amber-500 text-white px-2 py-1 rounded">Spotify</span>
                      )}
                      {selectedTrack.youtubeUrl && (
                      <span className="text-xs bg-amber-600 text-white px-2 py-1 rounded">YT</span>
                      )}
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-3">
                    <span className="bg-gray-800/50 text-gray-100 px-3 py-1 rounded-full text-sm">
                      {selectedTrack.customGenre || selectedTrack.genre}
                    </span>
                    {selectedTrack.duration && (
                      <span className="bg-gray-800/50 text-gray-200 px-3 py-1 rounded-full text-sm">
                        {selectedTrack.duration}
                      </span>
                    )}
                    {selectedTrack.year && (
                  ...
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold text-[var(--off-white)]">My Role</h3>
                    <p className="text-gray-200 font-medium">{selectedTrack.role}</p>
                  </div>

                  {selectedTrack.description && (
                    <div className="space-y-3">
                      <h3 className="text-lg font-semibold text-[var(--off-white)]">Details</h3>
                      <p className="text-gray-200 leading-relaxed">{selectedTrack.description}</p>
                    </div>
                  )}
                  ...
                  <div className="flex gap-4">
                    {selectedTrack.spotifyUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="service-button inline-flex items-center gap-3 bg-amber-600 hover:bg-amber-700 text-white px-6 py-3 rounded-xl font-medium transition-colors"
                      >
